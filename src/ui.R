...
# Links
useeio_url <- a("(USEEIO)", href = "https://www.epa.gov/land-research/us-environmentally-extended-input-output-useeio-models")
naics_url <- a("(NAICS)", href = "https://www.census.gov/naics/?input=&year=2012")

# Instructions

# Home
p1 <- tagList("This web application estimates the economic, greenhouse gas, and 
...
p3 <- "Manual Entry - which allows individually entered inputs of demand for known industries."

p4 <- "File Upload - which requires a comma, semicolon, or tab separated text file 
      which contains text descriptors of demand in one column and costs in another. 
      A template for this file will be provided on the File Upload tab."

p5 <- "Manual Entry can be useful in examining the impacts of a number of already-known industries, 
      while File Upload is best if you have many entries, or do not know what industry to map inputs to."

p6 <- "Please select the year for your economic inputs. The amount entered will 
      be converted from U.S. dollars for the selected year to 2012 U.S. dollars. 
      The results are always given in 2012 U.S. dollars. "
...
p_upload3 <- "You may keep or change the industry used for each input. If all 
              ...
              although the algorithm is designed to recognize many common acronyms 
              from hydrogen production literature, this recognition is not comprehensive."

p_upload4 <- tagList("Another table will appear below the table with the uploaded
                     ...
                     naics_url, "industries. This is meant to guide you in selecting 
...
p_man3 <- "For each desired industry, click on the industry dropdown to browse 
           the available industries. Enter the corresponding amount (in U.S. dollars) 
           in the corresponding cost entry box."
...
p_econ5 <- "You can switch between displaying the top 5 and top 10 industries 
            ...
            all industries not in these top industries (All Other N Industries).  "
...
p_ghg1 <- "This tab shows the greenhouse gas (GHG) emissions and global warming 
          ...
          either the manual or file upload method. The download everything button
          will download a .zip file with all of the .csv file downloads from the 
          Economics Impacts, GHG Impacts, and Other Environmental Impacts tabs. "

p_ghg2 <- HTML(paste0("This table shows the GWP impacts (kg CO", tags$sub("2"), 
                      ...
                      most to GWP impacts (kg CO", tags$sub("2"), "e, AR6, 100-yr) on top."))
  
p_ghg3 <- HTML(paste0("These plots show the GWP impacts (kg CO", tags$sub("2"), "e, AR6)
                      for each industry of the 5 (or 10) industries with the most 
                      total GWP impacts (kg CO", tags$sub("2"), "e, AR6, 100-yr)), 
                      as well as that of all industries not in these top industries 
                      (All Other N Industries), using first the 100- and then the 
                      20- year timeframe. You can switch between displaying the 
                      top 5 and top 10 industries using the Top 5 Industries and Top 10 
                      Industries buttons. The plots are organized with the 
                      industries which contribute most to GWP impacts (kg CO", 
                      tags$sub("2"), "e, AR6, 100-yr) on the bottom and those 
                      with the least on the top, except for All Other N Industries, 
                      which will always appear on top for consistency. The two plots 
                      have the industries arranged in the same order to allow 
                      for easy comparison. Place the pointer over a bar in the plots 
                      to reveal the specific number."))

p_other <- "This tab shows the acidification, eutrophication, ozone depletion, 
            and smog formation potentials of the inputs to demand industries using either 
            the manual or file upload method.   "
      ...
      tabsetPanel(id = "tabs",
        ...
        tabPanel("File Upload",
          ...
          sidebarPanel(
            ...
            fileInput("file1", 
              "Please upload a file with equipment in the first column and costs 
               in the second. Click these buttons to indicate the type of file.",
               multiple = FALSE,
...
# 
               ...
               # Horizontal line ----
               tags$hr(),
               a(href = "Example Upload.csv", 
                 "Download Example Template (CSV) File", 
                 download = NA, 
                 target = "_blank"),
        ),
          ...
          span("You can upload a file of one of the chosen types indicated on 
               the sidebar of the File Upload tab. Make sure to select the appropriate separator"), 
          strong("before"), 
          span("uploading the file. You can use the attached file Download Example Template (CSV) File 
               as an example and update it with your desired inputs. 
               Please ensure that the entries in the cost column are formatted 
               as numbers and not dollars. Blank entries in the cost column
               will be treated as zeros."),
          strong("Please note"),
          span("that all File Uploads must include headers for the tool to work 
               properly. A use case for such a file upload is a bill of materials 
               for a facility with equipment names (e.g., pumps) and their associated costs."),  
          br(), 
          br(),
          p(p_upload2),
          p(p_upload3),
        ...
        tabPanel("Manual Entry",
          ...
          actionButton("enter", "Show Results")
           ...
           # Bullet points
           tags$ul(
             tags$li("Resorting by other columns - Click on the headers to reorder the table based on that column"), 
             tags$li("Change the number of rows displayed in the table – Change the selection for “Show N entries”"), 
             tags$li("Scroll through additional rows – Click buttons under the table"),
         ...
         tabPanel("Other Environmental Impacts",
           # Display data table
